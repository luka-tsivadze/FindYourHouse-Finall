...
function getCacheKey(source, tileCoord) {
  return "".concat(source.getKey(), ",").concat(source.getRevision(), ",").concat(getKey(tileCoord));
}
...
function parseIconProperties(style, builder, uniforms, vertContext, fragContext) {
  ...
  if ("icon-opacity" in style) {
    color = "".concat(color, " * vec4(1.0, 1.0, 1.0, ").concat(expressionToGlsl(fragContext, style["icon-opacity"], NumberType), ")");
  }
...
function parseLiteralStyle(style, variables, filter) {
  ...
  for (const varName in fragContext.variables) {
    ...
    builder.addUniform("".concat(glslType, " ").concat(uniformName));
  ...
  function defineSpecialInput(contextPropName, glslPropName, type, callback) {
    ...
    builder.addAttribute("".concat(glslType, " a_").concat(glslPropName));
    if (inFragContext) {
      builder.addVarying("v_".concat(glslPropName), glslType, "a_".concat(glslPropName));
    }
...
function pushCustomAttributesInRenderInstructions(renderInstructions, customAttributes, batchEntry, currentIndex) {
  ...
  for (const key in customAttributes) {
    ...
    renderInstructions[currentIndex + shift++] = (_a = value == null ? void 0 : value[0]) != null ? _a : value;
